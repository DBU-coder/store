...
class UserProfileView(TitleMixin, UpdateView):
    ...
    def get_success_url(self):
        return reverse_lazy('users:profile', args=[self.request.user.id])  # или self.object.id
...
# @login_required  # декоратор запускает ф-ю только если пользователь зарегистрирован.
# def profile(request):
# 	user = request.user
# 	if request.method == 'POST':
